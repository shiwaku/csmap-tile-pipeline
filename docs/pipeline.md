...
| **タイル生成が異常に遅い（WSL）** | **出力先が `/mnt/c` などのWindowsドライブになっている。WSL内のext4に出力すると2倍以上速い（後述）** |

---

## WSL で実行する場合の注意

**出力先を Windows ドライブ（`/mnt/c` 等）にしてはいけない。**
タイル生成は小さいファイルを大量に書くため、WSLとWindows間のファイルシステム変換
（DrvFs）のオーバーヘッドが致命的に効く。

同一データ（青ヶ島 3,456枚）での実測:

| 出力先 | 速度 |
|---|---|
| `/mnt/c`（Windows） | 75〜90枚/秒 |
| **WSL内 ext4（`~/...`）** | **157〜213枚/秒（約2.4倍）** |

全23データセット（1,349万枚）では **約30時間 → 約13時間** の差になる。

削除も同様に遅い。約90万ファイルの `rm -rf` に10分近くかかった。
...
生成後にWindows側やサーバへ転送する場合は、`tar` でまとめてから移すと速い。
